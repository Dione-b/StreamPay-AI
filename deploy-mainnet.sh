...
# 🚀 Script de Deploy Automático para Mainnet StreamPay AI
# Este script faz deployment seguro sem alterar código

set -e

FRONTEND_DIR="/home/jistriane/Documentos/StreamPay AI/StreamPay-AI-1/frontend"
...
# 3. Validar build
echo -e "${YELLOW}[3/5] Validando build local (segurança)...${NC}"
cd "$FRONTEND_DIR"
...
echo -e "${GREEN}✅ Build validado com sucesso${NC}"
echo ""
...
if [ -z "$(git status --porcelain)" ]; then
    ...
    if [[ $REPLY =~ ^[Ss]$ ]]; then
        git add -A
        git commit -m "feat: update for mainnet deployment" || true
    else
